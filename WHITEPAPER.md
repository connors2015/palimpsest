...
3. **Canary and trigger probing as validator duty.** Committee scoring (§5.2) includes probes beyond loss: behavioral canaries, trigger-pattern sweeps, and regression suites on protected capabilities — a delta that moves canaries fails inclusion regardless of its loss score.
...
**Residual**: a stealthy backdoor that passes probes, survives the window, and is never detected is not excised. Detection quality is the open research frontier (§12.3).
...
>
> **Phase 0 (fourth pass): the primitives fused — a MoE transformer, on-chain.** `rig/moe_transformer.py` replaces every FFN block of the deep transformer with a mixture of experts (top-k routing, gradient-checked to 1e-8) and trains it *through the chain*: 6 miners drive a 2-layer × 8-expert model to 1.0 accuracy via DiLoCo aggregation, replaying bit-exact. Weights are paged as a backbone plus one page per (layer, expert); a **decode step** — advancing the sequence by one token, KV-cache style — routes that token to only top-k experts per layer (4 of 16 in the demo), so the serving node loads just the backbone + those expert pages and proves them with Merkle inclusion proofs against the committed root (tampered pages and wrong roots rejected). Per-token serving cost is O(top_k), not O(E): at 1024 experts/layer a decode step touches 0.2% of expert capacity. The suite is 64 tests (`scripts/run test`). Still toy-scale — these validate mechanisms, systems plumbing, and the sparse-serving primitive at architecture level, not model quality, and none yet exercise the hardest open problem (§12.3).
...
5. A stealthy-backdoor construction defeats probes + influence audit *reliably and cheaply* in the rig's red-team suite (reduces §7.2 to theater — the one word this design must never earn).
