"""Ed25519 — the public-domain reference implementation (D. J. Bernstein et al.).

Pure Python, no dependencies, so the rig stays self-contained. It is the slow
reference (bit-by-bit scalar multiply) — fine for a toy with a handful of
signatures per block, NOT for production (use libsodium/pynacl there). Kept
verbatim-in-spirit from the ed25519.cr.yp.to reference so it is auditable
