...
def validate_block(block: Block, parent_w_int: np.ndarray) -> np.ndarray:
    """Validate a block from first principles against its parent state.

    Returns the post-state weights if valid; raises ValidationError otherwise.
    Any node can run this — no trust in the proposer required (§5)."""
    h = block.header
...
class BlockTree:
    ...
    def add_block(self, block: Block) -> bool:
        ...
        w = validate_block(block, self.state[parent])              # may raise
...
def build_block(tree: BlockTree, parent_hash: str, accepted: list, bodies: dict,
                works: dict, proposer: str) -> Block:
    """Assemble a valid block extending `parent_hash` from accepted txs.
    ...
    return Block(header, accepted, {t.da_pointer: bodies[t.da_pointer] for t in accepted})
